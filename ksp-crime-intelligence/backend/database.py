...
def query_firs(fir_id: Optional[int] = None, crime_type: Optional[str] = None,
    ...
    params = []
    if fir_id:
        sql += " AND fir_id = ?"; params.append(fir_id)
    if crime_type:
        sql += " AND crime_type LIKE ?"; params.append(f"%{crime_type}%")
    if location:
        sql += " AND location LIKE ?"; params.append(f"%{location}%")
    if district:
        sql += " AND district LIKE ?"; params.append(f"%{district}%")
    if status:
        sql += " AND status = ?"; params.append(status)
    sql += " ORDER BY date DESC LIMIT ?"; params.append(limit)
...
def query_persons(person_id: Optional[int] = None, name: Optional[str] = None):
    ...
    if name:
        sql += " AND name LIKE ?"; params.append(f"%{name}%")
    cursor.execute(sql, params)
    rows = [dict(r) for r in cursor.fetchall()]
    conn.close()
    return rows
...
def query_involvements(fir_id: Optional[int] = None, person_id: Optional[int] = None):
    ...
    params = []
    if fir_id:
        sql += " AND i.fir_id = ?"; params.append(fir_id)
    if person_id:
        sql += " AND i.person_id = ?"; params.append(person_id)
    cursor.execute(sql, params)
    rows = [dict(r) for r in cursor.fetchall()]
    conn.close()
    return rows
...
def get_stats_crimes_by_type():
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT crime_type, COUNT(*) as count FROM firs GROUP BY crime_type ORDER BY count DESC")
    rows = [dict(r) for r in cursor.fetchall()]
    conn.close()
    return rows
